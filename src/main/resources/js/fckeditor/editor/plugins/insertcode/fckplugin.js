FCKCommands.RegisterCommand('InsertCode', new FCKDialogCommand('InsertCode',
				FCKLang["InsertCodeBtn"], FCKPlugins.Items['insertcode'].Path
						+ 'fck_insertcode.html', 500, 300));
var oInsertCode = new FCKToolbarButton('InsertCode', FCKLang["InsertCodeBtn"],
		FCKLang["InsertCodeBtn"], FCK_TOOLBARITEM_ONLYTEXT, false, true);

FCKToolbarItems.RegisterItem('InsertCode', oInsertCode);

var FCKInsertCode = new Object();
FCKInsertCode.Add = function(name,dfmt) {
	...
	// ht.Highlight(txt);
	// coText.innerHTML='<div
	// class="codeHead">'+lantxt+'代码</div>'+ht.div.innerHTML;
		coText.innerHTML = '<input type="text" name="'+name+'" class="ss_datepick" readonly="readonly" dfmt="'+dfmt+'"/>';
}
FCKInsertCode.InsertConstantCode = function(ckstr) {
	var coText = FCK.CreateElement('DIV');
	coText.innerHTML = ckstr;
}
var dp = {
	sh : {
		Utils : {},
...
dp.sh.Highlighter.prototype.ProcessSmartTabs = function(code) {
	...
	function InsertSpaces(line, pos, count) {
		...
		var right = line.substr(pos + 1, line.length); // pos + 1 will get rid
														// of the tab
		var spaces = '';
...
dp.sh.Brushes.CSharp = function() {
	...
	this.regexList = [
			...
			// { regex: new
			// RegExp('(?<!/)//(?!/).*$|(?<!/)////(?!/).*$|/\\*[^\\*]*(.)*?\\*/',
			// 'gm'), css: 'comment' }, // one line comments starting with
			// anything BUT '///' and multiline comments

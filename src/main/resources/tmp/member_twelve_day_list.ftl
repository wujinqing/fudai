...
<body class="mainbody">
<div class="fastNav">工作平台 > 营养师部工作平台 > 启动期12天客户</div>
<@u.search_bar action="member!twelveDayList.action">
	...
	<tr>
      <td colspan='10'>
    	 ...
    	 <select id="org" name="member.limitOrgs" onchange="getDept(this.value,1)">
	      ...
	      <#else>
		      机构：${(member.nutrOrgName)!}
		      部门：${(member.nutrDeptName)!}
	      </#if>
	                营养师： 
    	  <select id="staff" name="member.staff.id">
		    ...
		    <#if staffList??>
			    <#list staffList as staff> 
			       <option value="${staff.id}" <#if "${staffId!}"="${(staff.id)!}">selected</#if>>${(staff.name)!}</option>
			    </#list>
		    </#if>
		  </select>
      </td>
...
<form action="../cm/service!save.action?param=input" method="post" id="hideForm" name="form">
	<input type="hidden" id="memberId" name="service.member.id" />
	<input type="hidden" id="orgId"  name="service.org.id" />
	<input type="hidden" name="service.comments" value="取消启动期12天提醒。" />
</form>
...
<@u.paginate page=page/>
 ...
 <script type="text/javascript">
	...
	function showTasks(id){
		//var url = "../cm/service.action?memberId="+id;

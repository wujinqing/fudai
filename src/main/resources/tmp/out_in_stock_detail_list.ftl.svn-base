...
<html>
    ...
    <body class="mainbody">
     	<@u.search_bar action="out_in_stock!getDetailList.action">
        ...
        <@u.title_bar title="出入库明细统计表<font color='red'>（'-'表示库存无变化）</font>"></@u.title_bar>
        ...
        <table border="0" cellspacing="0">
            ...
            <#if (outInStockList??&&outInStockList?size gt 0)>
        	    			...
        	    			<#if (ois.oisOwnList??&&(ois.oisOwnList)?size gt 0)>
		        	    		...
		        	    		<#list ois.oisOwnList as own>
		        	    			<#if (own.product??&&own.product.id??&&pd.id??&&own.product.id=pd.id)>
		        	    				<td title="${pd.name!}"><a href="javascript:look('${(outInStock.type)!}','${(outInStock.startDate?string('yyyy-MM-dd'))!}','${(outInStock.endDate?string('yyyy-MM-dd'))!}','${(ois.org.id)!}','${(ois.orderType)}','${(pd.id)!}')">${own.num!}</a></td>
		        	    				<#assign flag=true>
		        	    				<#break>
		        	    			</#if>
		        	    		</#list>
            	...
            	<#else>
            	<tr>
            		<td colspan="<#if productList?size??>${(productList.size()+5)!}<#else>5</#if>">暂无记录</td>
            	</tr>
            </#if>
            <tr>
            	<td colspan="5">合计</td>

...
// Node object
function Node(id, pid, name, url, isChecked, title, target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
...
dTree.prototype.add = function(id, pid, name, url, isChecked, title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url, isChecked, title, target, icon, iconOpen, open);
};
...
dTree.prototype.getCookie = function(cookieName) {
	var cookieValue = '';
	var posName = document.cookie.toString().indexOf(escape(cookieName) + '=');
	if (posName != -1) {
		var posValue = posName + (escape(cookieName) + '=').length;
		var endPos = document.cookie.toString().indexOf(';', posValue);
		if (endPos != -1) cookieValue = unescape(document.cookie.toString().substring(posValue, endPos));
		else cookieValue = unescape(document.cookie.toString().substring(posValue));
	}
	return (cookieValue);
};
